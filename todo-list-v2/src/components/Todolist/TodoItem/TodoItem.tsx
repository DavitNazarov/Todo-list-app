...
export const TodoItem :React.FC<TodoItemProps> = ({todo ,checkTodo ,deleteTodo ,selectTodoIdForEdit}) => {
  return (
    ...
    <div>
        <div aria-hidden style={{opacity:todo.checked ? 0.5 : 1, textDecoration:todo.checked ? 'line-through': 'none' }} className={styles.todo_item_title} onClick={()=>checkTodo(todo.id)}>
          {todo.name}
        </div>
    </div>
    <div>
